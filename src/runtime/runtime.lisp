...
(defun execute-runtime-turn
  ...
  (let* ((state
    ...
    ;; Phase-B projection
    (let ((prompt
           (phase-b.view:project-to-prompt context)))
  ...
  (let* ((updated-state
         ...
         (latest-entry
          (phase-b.view:view-last updated-context)))

    (when (and latest-entry
               (eq (chron-llm.kernel:history-entry-kind latest-entry)
                   :assistant))

      (format t
              "~&AI> ~A~%"
              (chron-llm.kernel:history-entry-text
               latest-entry))
